...
const Sidenav = ({ isMobile }) => {
  ...
  return (
    <>
      {isMobile ? (
        <div className="fixed bottom-0 left-0 right-0 bg-[#1F1F1F] z-50">
          <button 
            onClick={toggleNav}
          ...
          {isOpen && (
            <div className="absolute bottom-full left-0 right-0 bg-[#1F1F1F] p-4 border-t border-zinc-700 max-h-[80vh] overflow-y-auto">
              <NavContent />
            </div>
          )}
        </div>
      ) : (
        <div className="w-64 h-full border-r border-zinc-700 p-6 bg-[#1F1F1F] overflow-y-auto">
          <NavContent />
        </div>
      )}
    </>
